...
const currentDir = `${process.cwd()}/test/visual/current`;
const baselineDir = `${process.cwd()}/test/visual/baseline`;
  ...
  for (prefix in config.viewports) {
    ...
    describe(prefix, function () {
      ...
      for (const name of config.elements) {
        it(`/test/visual/${name}.html`, async function () {
...
async function takeAndCompareScreenshot(page, route, filePrefix) {
  ...
  await page.goto(`http://127.0.0.1:${port}/test/visual/${route}.html`, { waitUntil: ['load', 'networkidle0'] });
